...
| `max_fail_frames` | 2 | Consecutive tracking-frame KCF failures before removing a track |
...
| `lookback_frames` | 60 | Frame buffer size for backward tracking when a new object is detected |
...
| 1 | TRACK | KCF `update()` succeeds -> box coordinates updated | Box follows object smoothly |
| 2 | TRACK | KCF `update()` succeeds | Box follows |
| 3 | TRACK | KCF `update()` succeeds | Box follows |
| 4 | TRACK | KCF `update()` succeeds | Box follows |
| 5 | DETECT | YOLO re-detects face, matched to Track 0 via Hungarian IoU matching -> fd reset to 0, box blended (0.7×YOLO + 0.3×KCF), KCF reinitialized on blended box | Box moves smoothly to new position |
...
| 1 | TRACK | KCF `update()` **fails** (object left frame) -> `frames_since_fail` = 1 | Box stays at last known position |
| 2 | TRACK | KCF `update()` **fails** again -> `frames_since_fail` = 2 | Box stays at last known position |
| 3 | TRACK | KCF `update()` **fails** again -> `frames_since_fail` = 3 > `max_fail_frames` (2) -> **track removed** | **Box disappears** |

**Takeaway:** When KCF fails, the track persists for up to `max_fail_frames` (2) consecutive failures, holding the last known box position. This prevents single-frame flicker from momentary tracking loss (e.g., motion blur, fast pan). Only sustained failure removes the track.
...
**Takeaway:** Coasting keeps a track alive for up to `max_coast_cycles` (4) detection cycles without a YOLO match. During coasting, KCF still updates the box position. The track is only checked for expiration on detection frames.
...
| 10 | DETECT | Face visible again, matched via Hungarian IoU -> fd reset to 0, box blended, KCF reinitialized | Box moves smoothly to fresh detection |
| 11+ | TRACK | Normal tracking resumes | Box follows smoothly |

**Takeaway:** Coasting bridges temporary detection gaps. When re-detected, the track seamlessly continues with the same track ID. If KCF also fails for more than `max_fail_frames` frames during the occlusion, the track is removed and a new track is created upon re-detection.
...
| 5 | DETECT | YOLO detects both faces. Hungarian matching assigns Face A to Track 0 (IoU). Face B has no match -> new Track 1 created. Backward tracking immediately rewinds through the buffer and adds blur boxes for frames 1-4 retroactively. | **2 boxes** (frame 5 onwards); **frames 1-4 retroactively blurred** |
