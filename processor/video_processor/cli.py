...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--tracker", choices=["kcf", "csrt"], default="csrt",
        help="Tracking algorithm to use between detection frames (default: csrt).",
    )
    parser.add_argument(
        "--report-format", choices=["csv", "ndjson"], default="csv",
        help="Format for the detection report written alongside the output (default: csv).",
    )
    parser.add_argument(
        "--no-video-output", action="store_true", default=False,
        help="Generate the detection report only; do not write a blurred output video.",
    )
