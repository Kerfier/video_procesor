...
describe('isAllowed', () => {
  ...
  it('allows upper-case extension', () => {
    expect(isAllowed(makeFile('VIDEO.MP4'))).toBe(true);
  });

  it('rejects non-mp4 formats', () => {
    expect(isAllowed(makeFile('clip.mov'))).toBe(false);
    expect(isAllowed(makeFile('movie.mkv'))).toBe(false);
    expect(isAllowed(makeFile('old.avi'))).toBe(false);
